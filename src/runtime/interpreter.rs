...
pub fn async_interpreter(
  ...
  Box::pin(async move {
    match node.clone().as_ref() {
      ...
      parser::Node::Await(a) => {
        let value = interpreter(a.expression.clone(), stack.clone(), modules.clone())?;
      ...
      parser::Node::Program(program) => {
        async_interpreter(program.body.clone().to_node().to_box(), stack, modules)
          .await?
          .to_result()
      }
...
pub fn interpreter(
  ...
  match node.clone().as_ref() {
    ...
    parser::Node::Call(call) => {
      ...
      let ret = callee.call(stack.clone(), this, args, modules.clone())?;
      if ret.is_return() {
