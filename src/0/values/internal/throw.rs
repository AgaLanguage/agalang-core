  ...
  pub fn get_error(&self) -> AgalError {
    match self {
      AgalThrow::Params {
        ...
        message,
        stack,
      } => AgalError::new(type_error.clone(), message.to_string(), stack.clone()),
      AgalThrow::Error(e) => e.clone(),
    }
  }
  pub fn from_ref_value(
    v: RefAgalValue<'a>,
    stack: &Stack,
    env: RefEnvironment<'a>,
  ) -> AgalThrow<'a> {
    let str = v.borrow();
    let str = str.to_agal_console(stack, env);
    if str.is_err() {
...
impl<'a> AgalValuable<'a> for AgalThrow<'a> {
  ...
  fn to_agal_string(&self, stack: &Stack, env: RefEnvironment) -> Result<AgalString, AgalThrow> {
    Ok(AgalString::from_string("<Error>"))
  }
  ...
  fn unary_back_operator(
    ...
    match operator {
      ...
      _ => crate::runtime::unary_back_operation_error(stack, operator, self.clone().to_ref_value()),
    }
  }
