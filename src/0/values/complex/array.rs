...
use crate::{
  runtime::{
    env::RefEnvironment, get_instance_property_error, get_property_error, AgalByte, AgalComplex,
    AgalNumber, AgalString, AgalThrow, AgalValuable, AgalValuableManager, AgalValue, RefAgalValue,
    Stack,
  },
  Modules,
};
...
impl<'a> AgalValuable<'a> for AgalArray<'a> {
  ...
  fn to_agal_console(
    ...
    let vec = self.get_vec().borrow();
    for (i, value) in vec.iter().enumerate() {
      let str = value.as_ref().borrow().to_agal_value(stack, env.clone());
      if str.is_err() {
        return str;
      }
