use crate::runtime::{
  binary_operation_error, env::RefEnvironment, get_instance_property_error, unary_operation_error,
  AgalArray, AgalBoolean, AgalPrimitive, AgalString, AgalThrow, AgalValuable, AgalValuableManager,
  ...
  fn binary_operation(
    ...
    match (_other, operator) {
      ...
      (AgalValue::Primitive(AgalPrimitive::Byte(other)), "-") => {
        ...
        if b > a {
          return binary_operation_error(stack, operator, (*self).to_ref_value(), _other_.clone());
        }
        let c = a - b;
        let byte1 = ((c >> 8) & 0xFF) as u8;
        let byte2 = (c & 0xFF) as u8;
